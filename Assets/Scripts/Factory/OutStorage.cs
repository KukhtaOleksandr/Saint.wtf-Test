...
namespace Factory
{
    public class OutStorage : MonoBehaviour
    {
        ...
        {
            return new Vector3(_leftBound.position.x + _cellSize.x / 2, _last.y + _cellSize.y, _forwardBound.position.z - _cellSize.z / 2);
        }

        private Vector3 GetFirstInNextRowPosition()
        {
            return new Vector3(_last.x + _cellSize.x, _last.y, _last.z);
        }

        private Vector3 GetFirstInRowPosition()
        {
            return new Vector3(_last.x + _cellSize.x / 2, _last.y, _last.z);
        }
